...
final class Client
{
     ...
     *
     * Deliberately unlike config(): never cached, never defaulted, and it throws
     ...
     *
     * @return array{key: string, format: string, value: mixed, version_id: string, created_at: string, created_by_id: string}
     */
    public function getSecret(string $key): array
    {
        $secret = $this->request('GET', '/server/secrets/' . $this->segment($key))['secret'] ?? null;
        if (!is_array($secret) || !array_key_exists('value', $secret)) {
            throw new FlagDashException("FlagDash: secret '{$key}' returned no value");
        }
        return $secret;
    }

    /** Returns just the decrypted value. Throws for the same reasons as getSecret(). */
    public function secret(string $key): mixed { return $this->getSecret($key)['value']; }
